...
# ── 廠區（Site）設定 ───────────────────────────────────────────────────────
# admin.json：
#   { "KH": ["K18251", ...], "KL": [...], "NL": [...],          ← 廠區代碼：該廠區管理者工號
#     "cross_site": { "K18251": ["KL", "NL"] } }                ← 選填：可跨廠區查閱的工號 → 允許進入的廠區
# 同一工號列在多個廠區的管理者名單，或列在 cross_site，登入時即可選擇要進入的廠區
# 所有資料檔皆放在 static/data/<廠區>/<廠區>_<檔名>
# 前端每一次請求都在 query string 帶 site 與 username（?site=KH&username=K18251）
ADMIN_FILE = "admin.json"
DEFAULT_SITE = "KH"   # 請求未帶 site 時使用的廠區
...
def _req_value(key):
    """依序從 query string → JSON body → form 取值"""
    try:
    ...
    except RuntimeError:  # 非請求上下文（模組載入時）
        return ""

def site_codes(cfg=None):
    """admin.json 內的廠區代碼（排除 cross_site 等設定鍵）"""
    cfg = cfg if cfg is not None else load_admin_sites()
    return [k for k in cfg.keys() if k != "cross_site"]

def current_site():
    site = _req_value("site").upper()
    return site if site in site_codes() else DEFAULT_SITE

def find_home_site(user_id, cfg=None):
    """依工號找本廠：先看各廠區管理者名單，再逐廠區找 <廠區>_Backend_data.json；找不到回 None"""
    cfg = cfg if cfg is not None else load_admin_sites()
    for site in site_codes(cfg):
        if user_id in [str(x).strip() for x in cfg.get(site, [])]:
            return site
    for site in site_codes(cfg):
        backend_file = os.path.join(DATA_DIR, site, f"{site}_Backend_data.json")
...
def allowed_sites(user_id, cfg=None):
    """該工號可進入的廠區：本廠 + 列名為管理者的廠區 + cross_site 設定的廠區"""
    cfg = cfg if cfg is not None else load_admin_sites()
    codes = site_codes(cfg)
    result = []
    home = find_home_site(user_id, cfg)
    if home:
        result.append(home)
    for site in codes:
        if user_id in [str(x).strip() for x in cfg.get(site, [])] and site not in result:
            result.append(site)
    for site in cfg.get("cross_site", {}).get(user_id, []):
        site = str(site).strip().upper()
        if site in codes and site not in result:
            result.append(site)
    return result

def current_username():
    return _req_value("username")

def site_admin_ids(site=None):
    """admin.json 中該廠區的管理者工號"""
    site = site or current_site()
    return [str(x).strip() for x in load_admin_sites().get(site, [])]

class SitePath(os.PathLike):
    """依 current_site() 動態產生廠區路徑，可直接給 open / pandas / os.path 使用
    - 所有廠區檔案皆放在 static/data/<廠區>/<廠區>_<檔名>
    """
    ...
    def __str__(self):
        site = current_site()
        name = f"{site}_{self._name}"
        return os.path.join(self._dir, site, name) if self._dir else name
    def __fspath__(self):
        return str(self)
    def __repr__(self):
        return str(self)
...
@app.before_request
def log_site_request():
    if request.method == "OPTIONS":
        return
    site, user = current_site(), current_username()
    logger.info(f"[{site}] [{user or '-'}] {request.method} {request.path}")
    # 跨廠區存取權限檢查：有帶 username 且要求的 site 不在其允許清單 → 403
    if user and request.path not in ("/api/getAllLoginer", "/api/login"):
        allowed = allowed_sites(user)
        if allowed and site not in allowed:
            logger.warning(f"⛔ [{site}] [{user}] 無權存取此廠區，允許：{allowed}")
            return jsonify({"error": f"無權存取 {site} 廠區資料"}), 403
# ─────────────────────────────────────────────────────────────────────────

# 讓 SitePath 進 jsonify 時自動轉成字串（例如回傳 "file": BUYER_FILE）
from flask.json.provider import DefaultJSONProvider
class SiteJSONProvider(DefaultJSONProvider):
    @staticmethod
    def default(o):
        if isinstance(o, SitePath):
            return str(o)
        return DefaultJSONProvider.default(o)
app.json = SiteJSONProvider(app)

# ══════════════════════════════════════════════════════════════════════
# 路徑設定（全部集中於此）
# ══════════════════════════════════════════════════════════════════════
DATA_DIR = "static/data"                 # 各廠區資料根目錄：static/data/<廠區>/<廠區>_<檔名>

# ── 分廠區檔案（SitePath，依請求的 site 動態切換）──
BACKEND_DATA          = SitePath(DATA_DIR, "Backend_data.json")       # 人員資料
CONFIG_FILE           = SitePath(DATA_DIR, "config.cfg")              # 需求者名單
CSV_FILE              = SitePath(DATA_DIR, "Planned_Purchase_Request_List.csv")  # 請購主表
BUYER_FILE            = SitePath(DATA_DIR, "Buyer_detail.csv")        # 請購明細 / eRT 總表
JSON_FILE             = SitePath(DATA_DIR, "money.json")              # 預算
PHONE_FILE            = SitePath(DATA_DIR, "phone.json")              # 分機
VENDER_FILE_PATH      = SitePath(DATA_DIR, "vender.ini")              # 廠商清單
ACC_MAIL_DATA_FILE    = SitePath(DATA_DIR, "accMailData.json")        # 驗收發信備註選項
MAIL_RECIPIENTS_FILE  = SitePath(DATA_DIR, "mailRecipients.json")     # 發送 Mail 固定人員
DELIVERY_RECEIPT_FILE = SitePath(DATA_DIR, "delivery_receipt.csv")    # 廠商交期上傳暫存（用完即刪）

# ── 全廠區共用（資料夾共用，檔名在使用處加廠區前綴）──
UPLOAD_DIR       = "uploads"                                # eHub / 狀態上傳暫存
PROCESSED_DIR    = "processed"                              # MHTML 解析暫存
FILTERS_DIR      = "user_filters"                           # 篩選狀態：<廠區>_<工號>_filters.json
MB_DIR           = "message_board_data"                     # 留言板
MB_CHANNELS_DIR  = os.path.join(MB_DIR, "channels")         #   <廠區>_<channel>.json
MB_VISITS_DIR    = os.path.join(MB_DIR, "visits")           #   <廠區>_<工號>.json
MB_LOG_FILE      = os.path.join(MB_DIR, "message_board.log")
BUYER_FILE_LOCK  = os.path.join(DATA_DIR, "Buyer_detail.csv.lock")   # 全廠區共用一把鎖

MAIL_DOMAIN = "@aseglobal.com"

for _d in [UPLOAD_DIR, PROCESSED_DIR, FILTERS_DIR, MB_DIR, MB_CHANNELS_DIR, MB_VISITS_DIR]:
    os.makedirs(_d, exist_ok=True)

# ── 啟動自檢：每個廠區資料夾必須有的檔案 ──
SITE_REQUIRED_FILES = [
    "Backend_data.json", "config.cfg", "Planned_Purchase_Request_List.csv", "Buyer_detail.csv",
    "money.json", "phone.json", "vender.ini", "accMailData.json", "mailRecipients.json",
]

def check_site_files(log=True):
    """檢查 admin.json 各廠區的資料夾與檔案是否齊全，回傳 {廠區: {"missing": [...], "misnamed": [...]}}"""
    report = {}
    try:
        cfg = load_admin_sites()
    except Exception as e:
        if log:
            logger.error(f"⛔ 讀取 {ADMIN_FILE} 失敗: {e}")
        return {"_error": str(e)}
    codes = site_codes(cfg)
    for site in codes:
        folder = os.path.join(DATA_DIR, site)
        missing, misnamed = [], []
        if not os.path.isdir(folder):
            missing = [f"{site}_{f}" for f in SITE_REQUIRED_FILES]
            report[site] = {"folder_exists": False, "missing": missing, "misnamed": []}
            if log:
                logger.error(f"⛔ [{site}] 資料夾不存在: {folder}")
            continue
        for f in SITE_REQUIRED_FILES:
            if not os.path.exists(os.path.join(folder, f"{site}_{f}")):
                missing.append(f"{site}_{f}")
        # 資料夾裡出現別的廠區前綴或沒前綴的檔案 → 很可能是命名錯誤
        for name in os.listdir(folder):
            if name.endswith(".lock"):
                continue
            prefix = name.split("_", 1)[0]
            if not name.startswith(f"{site}_") and (prefix in codes or name in SITE_REQUIRED_FILES):
                misnamed.append(name)
        report[site] = {"folder_exists": True, "missing": missing, "misnamed": misnamed}
        if log:
            if not missing and not misnamed:
                logger.info(f"✅ [{site}] 資料檔齊全 ({folder})")
            if missing:
                logger.error(f"⛔ [{site}] 缺少檔案: {', '.join(missing)}")
            if misnamed:
                logger.error(f"⚠ [{site}] 檔名前綴錯誤（應為 {site}_）: {', '.join(misnamed)}")
    return report
...
def getAllLoginer():
    try:
        ...
        sites = allowed_sites(user_id)
        if not sites:
            logger.warning(f"登入失敗，找不到工號: {user_id}")
            return jsonify({"error": "Item not found"}), 404

        logger.info(f"登入對應廠區: {user_id} → 本廠 {sites[0]}，可進入 {sites}")
        # site = 本廠（預設）；sites = 全部可進入的廠區，多於一個時由登入頁讓使用者選擇
        return jsonify({"name": "Username Find", "site": sites[0], "sites": sites, "username": user_id})
...
def save_filters_json():
    try:
        ...
        # 儲存到 JSON 檔案
        filename = os.path.join(FILTERS_DIR, f'{current_site()}_{username}_filters.json')
...
def get_data():
    if not os.path.exists(CSV_FILE):
        site = current_site()
        msg = f"找不到 {site} 廠區請購主表：{CSV_FILE}，請確認 static/data/{site}/ 內檔名前綴為 {site}_"
        logger.error(f"⛔ [{site}] {msg}")
        return jsonify({"error": msg, "site_check": check_site_files(log=False).get(site)}), 500
...
def get_unordered_count():
    ...
    if not os.path.exists(CSV_FILE):
        logger.error(f"⛔ [{current_site()}] 找不到請購主表: {CSV_FILE}（請確認 static/data/{current_site()}/ 內檔名前綴為 {current_site()}_）")
...
def api_site_check():
    """各廠區資料檔自檢結果（管理者排錯用）"""
    return jsonify(check_site_files(log=False))


@app.route('/api/admins', methods=['GET'])
def get_admins():
    """管理者名單只從 admin.json[廠區] 讀取"""
    try:
        return jsonify(site_admin_ids())
    except Exception as e:
        return jsonify({"error": str(e)}), 500
        ...
        for entry in users:
            if entry.get("工號", "").strip() == emp_id:
                return jsonify({
                    ...
                    "後台權限": "O" if emp_id in site_admin_ids() else "X"   # 權限以 admin.json 為準
                })
...
def check_edit_permission():
    try:
        ...
        # 管理者權限只看 admin.json[廠區]
        is_admin = current_user in site_admin_ids()
...
def sendmail():
    try:
        ...
        if data["data"]["請購順序"] == "1":
            ...
            try:
                print(data["data"], '\n', data["recipient"], '\n', mail_name, '\n', cc_string)
                send_mail(data["data"], data["recipient"], mail_name, cc_string, recipients_file=str(MAIL_RECIPIENTS_FILE))
                print("超急件郵件發送完成")
...
def save_csv():
    """
    save_csv V7 - 上傳 XLS 轉 CSV、依 PO 分群並偵測分批 / 合併 / 一般比對

    V7（2026/09/22）：
      - 分批 / 合併筆數比對只看「未結案」列（df_open：排除 驗收狀態 V，或 驗收數量 ≥ 數量；部分驗收仍算未結案）
      - XLS 讀入後先 strip PO / Item / 品名 / SOD / 交期，並移除完全重複列
      - 交期正規化：Excel 日期序號（45945 / 45945.0）→ yyyy/mm/dd；2025-10-15 等格式 → 2025/10/15；
        SOD 正規化：1.0 → 1、1,200 → 1200（手動編輯過的 XLS 常見）
      - 先扣掉「已知批次」：XLS 某列的 SOD + 交期 與 CSV 既有列完全相同（通知檔常把已結案批次再列一次）
      - 「合併」需同時滿足：待處理 XLS 筆數 < 待處理未結案批次數、都是分批列、總量相等
        → merge_items.xls_data 只帶待處理列，csv_data 只帶要被合併的未結案批次（已結案批次不動）
      - 「分批」：待處理 XLS 筆數 > 待處理未結案列數
        → xls_data 帶全部 XLS 列、csv_count / total_sod 含已結案批次（調整分批頁刪舊建新，驗收紀錄由
          confirm_quantity_update 帶到對應批次）
      - XLS 數量只等於其中一批（分批1 18 + 分批2 78，XLS 只列 78）→ 不判合併，交給一般比對 qty_match
    """
    if request.method == "OPTIONS":
        return jsonify({"status": "ok"}), 200
    ...
    # 🆕 V6：欄位前後空白清掉（PO 有空白會分群失敗、變成 0 筆），並移除完全重複的列（重複列會被誤判成分批）
    for _c in ("PO NO 採購單號碼", "PO Item 採購單項次", "Description 品名", "SOD Qty 廠商承諾數量", "Delivery Date 廠商承諾交期"):
        if _c in df_all.columns:
            df_all[_c] = df_all[_c].astype(str).str.strip()
    # 🆕 V7：手動編輯過的 XLS 會把交期存成 Excel 日期序號（45945）、數量存成 1.0 → 統一正規化
    def _norm_date(v):
        v = str(v).strip()
        m = re.fullmatch(r"(\d{5})(?:\.0+)?", v)
        if m:                                   # Excel 序號 → yyyy/mm/dd（1899/12/30 起算）
            from datetime import datetime, timedelta
            return (datetime(1899, 12, 30) + timedelta(days=int(m.group(1)))).strftime("%Y/%m/%d")
        m = re.fullmatch(r"(\d{4})[-./](\d{1,2})[-./](\d{1,2})(?:\s.*)?", v)
        if m:                                   # 2025-10-15 / 2025.10.15 / 2025/10/15 00:00 → 2025/10/15
    ...
    def _norm_qty(v):
        ...
        try:
            ...
            return v
    for _c in ("Delivery Date 廠商承諾交期", "Request Date 需求日期"):
        if _c in df_all.columns:
            df_all[_c] = df_all[_c].apply(_norm_date)
    if "SOD Qty 廠商承諾數量" in df_all.columns:
        df_all["SOD Qty 廠商承諾數量"] = df_all["SOD Qty 廠商承諾數量"].apply(_norm_qty)

    _before = len(df_all)
    df_all = df_all.drop_duplicates(subset=[c for c in ("PO NO 採購單號碼", "PO Item 採購單項次", "Description 品名", "SOD Qty 廠商承諾數量", "Delivery Date 廠商承諾交期") if c in df_all.columns]).reset_index(drop=True)
    if len(df_all) != _before:
        logger.warning(f"⚠️ XLS 有 {_before - len(df_all)} 列完全重複，已移除")
    ...
    try:
        with buyer_file_lock:
            ...
            # 🆕 V2：已驗收結案的列不參與分批 / 合併筆數比對（XLS 只會列出未結案的批次）
            _acc = pd.to_numeric(df_active["驗收數量"].str.replace(",", "").str.strip(), errors="coerce").fillna(0)
            _qty = pd.to_numeric(df_active["數量"].str.replace(",", "").str.strip(), errors="coerce")
            _closed = (
                df_active["驗收狀態"].str.strip().str.upper().eq("V")
                | ((_qty > 0) & (_acc >= _qty))          # 驗收數量 ≥ 數量 才算結案；部分驗收仍是未結案
            )
            df_open = df_active[~_closed].copy()
            logger.info(f"未結案資料(供分批/合併偵測): {len(df_open)}")
    ...
    for file_info in saved_files:
        ...
        buyer_related = df_open[df_open["PO No."].str.contains(po_no, regex=False, na=False)].copy()
        ...
        def _csv_qty_total(df_rows):
            """CSV 批次總量：SOD 為空時退回「數量」欄"""
            sod = _num_series(df_rows["SOD Qty 廠商承諾數量"])
            qty = _num_series(df_rows["數量"]) if "數量" in df_rows.columns else sod
            return sod.fillna(qty).fillna(0).sum()

        def _is_batch_row(r):
            return ("分批" in str(r.get("品項", ""))) or ("分批" in str(r.get("備註", "")))
        
        # 🆕 V5：同 PO 下「有效(狀態 V)」的全部列，含已結案批次；分批 / 合併判斷要同時看兩者
        active_related = df_active[df_active["PO No."].str.contains(po_no, regex=False, na=False)].copy()

        for item, xls_group in xls_item_groups:
            xls_rows_all = [{
                "description": str(r.get("Description 品名", "")),
                "delivery": str(r.get("Delivery Date 廠商承諾交期", "")).strip(),
                "sod_qty": str(r.get("SOD Qty 廠商承諾數量", "0")).strip()
            } for _, r in xls_group.iterrows()]
            xls_count = len(xls_rows_all)

            csv_all = active_related[active_related["Item"] == item]          # 含已結案
            csv_open = buyer_related[buyer_related["Item"] == item]           # 未結案
            csv_all_count, csv_open_count = len(csv_all), len(csv_open)
            ...
            # ① 先扣掉「已知批次」：XLS 某列的數量 + 交期 與 CSV 既有列完全相同（通知檔會把已結案批次再列一次）
            known_idx = set()
            xls_remaining = []
            for xr in xls_rows_all:
                hit = None
                for ci, cr in csv_all.iterrows():
                    if ci in known_idx:
                        continue
                    if _q(cr.get("SOD Qty 廠商承諾數量", "")) == _q(xr["sod_qty"]) and \
                       str(cr.get("Delivery Date 廠商承諾交期", "")).strip() == xr["delivery"]:
                        hit = ci
                        break
                if hit is not None:
                    known_idx.add(hit)
                else:
                    xls_remaining.append(xr)
            csv_open_remaining = csv_open[~csv_open.index.isin(known_idx)]
            n_xls_rem, n_open_rem = len(xls_remaining), len(csv_open_remaining)

            xls_rem_total = sum(_q(x["sod_qty"]) or 0 for x in xls_remaining)
            open_rem_total = _csv_qty_total(csv_open_remaining) if n_open_rem else 0.0
            batch_rows = csv_open_remaining[csv_open_remaining.apply(_is_batch_row, axis=1)] if n_open_rem else csv_open_remaining
            all_batch = n_open_rem >= 2 and len(batch_rows) == n_open_rem

            logger.info(f"   Item {item}: XLS {xls_count} 筆(已知 {len(known_idx)}, 待處理 {n_xls_rem}) / "
                        f"CSV 有效 {csv_all_count} 筆(未結案 {csv_open_count}, 待處理 {n_open_rem})")

            # ② 合併：待處理 XLS 筆數 < 待處理未結案批次數，且總量相等，且都是分批列
            is_real_merge = (n_open_rem > 0 and 0 < n_xls_rem < n_open_rem and all_batch
                             and abs(xls_rem_total - open_rem_total) < 0.01)
            # ③ 分批：待處理 XLS 筆數 > 待處理未結案列數（至少要有一列未結案）
            is_split = (n_open_rem > 0 and n_xls_rem > n_open_rem)

            if csv_all_count > 0 and n_xls_rem < n_open_rem and not is_real_merge:
                logger.info(f"⭕️ PO={po_no}, Item={item}: XLS 待處理 {n_xls_rem} 筆({xls_rem_total:g}) < 未結案 {n_open_rem} 筆({open_rem_total:g}) "
                            f"→ 不符合合併條件，視為批次更新")

            def _csv_row(r, extra=None):
                d = {"id": str(r.get("Id", "")), "description": str(r.get("品項", "")),
                     "delivery": str(r.get("Delivery Date 廠商承諾交期", "")),
                     "sod_qty": str(r.get("SOD Qty 廠商承諾數量", "0")), "note": str(r.get("備註", "")),
                     "total_qty": str(r.get("總數", "0")),
                     "closed": bool(r.name in df_active.index and r.name not in df_open.index)}
                if extra: d.update(extra)
                return d

            if is_real_merge:
                logger.info(f"🔄 偵測到合併:PO={po_no}, Item={item}, XLS 待處理 {n_xls_rem} 筆 < 未結案批次 {n_open_rem} 筆（總量 {open_rem_total:g}）")
                merge_po_set.add(po_no)
                merged_items.append({
                    "po_no": po_no,
                    "item": item,
                    "xls_count": int(n_xls_rem),
                    "csv_count": int(n_open_rem),
                    "xls_data": xls_remaining,                       # 只給「待處理」的 XLS 列
                    "csv_data": [_csv_row(r) for _, r in csv_open_remaining.iterrows()],
                    "kept_data": [_csv_row(r) for i, r in csv_all.iterrows() if i in known_idx or i not in csv_open.index],
                    "action_type": "merge"
                })
                logger.info(f"   📝 已記錄需要合併的項目")

            elif is_split:
                mismatch_po_set.add(po_no)
                total_sod = _csv_qty_total(csv_all)                  # 原 SOD 總量：含已結案批次
                quantity_mismatch_items.append({
                    "po_no": po_no,
                    "item": item,
                    "xls_count": int(xls_count),
                    "csv_count": int(csv_all_count),
                    "total_sod": float(total_sod),
                    "xls_data": xls_rows_all,                        # 全部 XLS 列（調整分批頁會刪舊建新）
                    "csv_data": [_csv_row(r) for _, r in csv_all.iterrows()]
                })
                logger.info(f"⚠️ 筆數不符(分批): PO={po_no}, Item={item}, XLS={xls_count}筆, CSV={csv_all_count}筆")
...
def confirm_quantity_update():
    ...
    confirm_quantity_update V4（2026/09/22）：
      - 新分批列的品項若未含「分批」標記，自動補上「-分批N」（與反向分批一致，後續比對才認得出批次）
      - 每一筆已有驗收的舊列，各自把 驗收數量 / 驗收狀態 / 交貨驗證 / 發票月份 / RT金額 / RT總金額
        帶到「SOD 與驗收數量相同」且未被認領的新批次（找不到就依序帶到下一批），多批已驗收也不會遺失
      - 原備註只保留在承接驗收紀錄的那一批（無驗收則第一批），其餘批次僅寫「分批N/M」
      - 同 PO 已有其他 Item 的分批列（如反向分批留下的分批1）時，新批次接續編號，並更新既有列的 /M
    """
    ...
    try:
        ...
        try:
            with buyer_file_lock:
                ...
                logger.info(f"📋 取得第一筆舊資料作為範本")
                first_old_row = df_buyer[old_mask].iloc[0]
                old_rows_snapshot = df_buyer[old_mask].copy()   # 🆕 V3：刪除前先留一份，供驗收紀錄承接
                ...
                # 🆕 V2：原列已有部分驗收 → 找出要承接驗收紀錄的批次
                def _num(v):
                    try:
                        return float(str(v).replace(",", "").strip())
                    except (ValueError, TypeError):
                        return None
                # 🆕 V3：每一筆已有驗收的舊列，各自找一筆「SOD 相同」且尚未被認領的新批次承接（多批已驗收也能對上）
                carry_map = {}   # 新批次 idx -> 舊列 Series
                for _, _old in old_rows_snapshot.iterrows():
                    _acc = _num(_old.get("驗收數量", "")) or 0.0
                    if _acc <= 0:
                        continue
                    _target = None
                    for _i, _r in enumerate(new_rows):
                        if _i in carry_map:
                            continue
                        if _num(_r.get("sod_qty", "")) == _acc:
                            _target = _i
                            break
                    if _target is None:
                        for _i in range(len(new_rows)):
                            if _i not in carry_map:
                                _target = _i
                                break
                    if _target is not None:
                        carry_map[_target] = _old
                        logger.info(f"📎 舊列「{_old.get('品項','')[:30]}」已驗收 {smart_number_format(_acc)} 件 → 帶到第 {_target+1} 批")
                carry_idx = min(carry_map) if carry_map else None

                # 🆕 V4：同 PO、同品項基底、不同 Item 的既有分批列（例如反向分批留下的 分批1）→ 編號接續，不重複「分批1」
                def _base_name(desc):
                    d = re.sub(r"[-\s]*\(?分批[^)\s]*\)?", "", str(desc))
                    d = re.sub(r"[-\s]*尾批", "", d)
                    return re.sub(r"\s*\(\d+\s*Set\)", "", d).strip()
                _base = _base_name(first_old_row.get("品項", ""))
                _sib_mask = (
                    (df_buyer["開單狀態"] == "V")
                    & (df_buyer["PO No."].str.strip() == po_no)
                    & df_buyer["品項"].apply(lambda x: _base_name(x) == _base)
                    & (df_buyer["品項"].str.contains("分批", na=False) | df_buyer["備註"].str.contains("分批", na=False))
                )
                seq_offset = int(_sib_mask.sum())
                seq_total = seq_offset + len(new_rows)
                if seq_offset:
                    logger.info(f"🔢 同 PO 已有 {seq_offset} 個其他 Item 的分批列 → 新批次從 分批{seq_offset+1} 開始編號，總數 {seq_total}")
                    for _i in _sib_mask[_sib_mask].index:
                        df_buyer.at[_i, "備註"] = re.sub(r"分批(\d+)/\d+", lambda m: f"分批{m.group(1)}/{seq_total}", str(df_buyer.at[_i, "備註"]))
                old_note = str(first_old_row.get("備註", "")).strip()
                old_note = re.sub(r"；?分批\d+/\d+", "", old_note).strip("；").strip()
                ...
                for idx, row_data in enumerate(new_rows):
                    ...
                    # 🆕 V2：品項補「-分批N」；備註保留原內容
                    _desc = str(row_data.get("description", "") or first_old_row.get("品項", "")).strip()
                    _desc = re.sub(r"[-\s]*分批\s*\d*(?:/\d+)?\s*$", "", _desc)
                    if seq_total > 1:
                        _desc = f"{_desc}-分批{seq_offset + idx + 1}"
                    _tag = f"分批{seq_offset + idx + 1}/{seq_total}" if seq_total > 1 else ""
                    _carry = (idx in carry_map)
                    _src = carry_map.get(idx, first_old_row)          # 承接來源列
                    _keep_note = (idx == (carry_idx if carry_idx is not None else 0))   # 原備註只留在承接批次
                    _note = ((old_note + "；" if old_note and _tag else old_note) if _keep_note else "") + _tag

                    new_row = {
                        "Id": first_old_row.get("Id", ""),
                        "開單狀態": "V",
                        "交貨驗證": _src.get("交貨驗證", "") if _carry else "",
                        "User": first_old_row.get("User", ""),
                        "ePR No.": first_old_row.get("ePR No.", ""),
                        "PO No.": po_no,
                        "Item": item,
                        ...
                        "Delivery Date 廠商承諾交期": row_data.get("delivery", ""),
                        "SOD Qty 廠商承諾數量": sod_qty_formatted,
                        "驗收數量": _src.get("驗收數量", "") if _carry else "",
                        "拒收數量": _src.get("拒收數量", "") if _carry else "",
                        "發票月份": _src.get("發票月份", "") if _carry else "",
                        "WBS": first_old_row.get("WBS", ""),
                        "需求日": first_old_row.get("需求日", ""),
                        "RT金額": _src.get("RT金額", "") if _carry else "",
                        "RT總金額": _src.get("RT總金額", "") if _carry else "",
                        "驗收狀態": (_src.get("驗收狀態", "") or "X") if _carry else "X"
                    }
                    new_data_rows.append(new_row)
                    logger.info(f"      品項: {new_row['品項']}, SOD: {new_row['SOD Qty 廠商承諾數量']}, 總價: {new_row['總價']}")
...
def confirm_merge():
    ...
    confirm_merge V2（2026/09/22）：
      - 只刪除「未結案」的分批列（驗收狀態 V 或 驗收數量 ≥ 數量 的批次保留不動）
      - 分批列判斷：品項或備註含「分批」
      - 合併後若仍有其他批次（已結案的、或 XLS 仍給多筆）→ 新列品項補「-分批N」、備註「分批N/M」接續編號；
        合併成單一列時品項去掉分批標記、備註只清掉「分批N/M」
    """
            ...
            with buyer_file_lock:
                ...
                def _num(v):
                    try:
                        ...
                        return None
                _acc = pd.to_numeric(df_buyer["驗收數量"].str.replace(",", "").str.strip(), errors="coerce").fillna(0)
                _qty = pd.to_numeric(df_buyer["數量"].str.replace(",", "").str.strip(), errors="coerce")
                _closed = df_buyer["驗收狀態"].str.strip().str.upper().eq("V") | ((_qty > 0) & (_acc >= _qty))
                _same_item = (df_buyer["PO No."].str.strip() == po_no) & (df_buyer["Item"].str.strip() == item) & (df_buyer["開單狀態"] == "V")
                _is_batch = df_buyer["備註"].str.contains("分批", na=False) | df_buyer["品項"].str.contains("分批", na=False)

                # 🆕 V2：只刪未結案的分批列；已結案批次保留
                old_mask = _same_item & _is_batch & ~_closed
                kept_mask = _same_item & _closed
                kept_count = int(kept_mask.sum())
                logger.info(f"📌 已結案批次保留 {kept_count} 筆")
                ...
                total_after = kept_count + len(xls_data)
                for idx, row_data in enumerate(xls_data):
                    logger.info(f"   處理第 {idx+1}/{len(xls_data)} 筆新資料...")
                    ...
                    # 🆕 V2：清理備註（任何「分批N/M」）、決定新列的品項 / 備註
                    old_note = re.sub(r"；?分批\d+/\d+", "", str(first_old_row.get("備註", ""))).strip("；").strip()
                    base_desc = re.sub(r"[-\s]*分批\s*\d*(?:/\d+)?\s*$", "", str(row_data.get("description", "") or first_old_row.get("品項", "")).strip())
                    if total_after > 1:
                        seq = kept_count + idx + 1
                        new_desc = f"{base_desc}-分批{seq}"
                        new_note = (old_note + "；" if old_note else "") + f"分批{seq}/{total_after}"
                    else:
                        new_desc = base_desc
                        new_note = old_note
                ...
                # 🆕 V2：保留的已結案批次，備註「分批N/M」的 M 更新為合併後總批數
                if kept_count and total_after > 1:
                    for _i in kept_mask[kept_mask].index:
                        if _i in df_buyer.index:
                            df_buyer.at[_i, "備註"] = re.sub(r"分批(\d+)/\d+", lambda m: f"分批{m.group(1)}/{total_after}", str(df_buyer.at[_i, "備註"]))
...
def get_notes_prefix(user_name, backend_file=BACKEND_DATA):
    with open(backend_file, "r", encoding="utf-8-sig") as f:
        data = json.load(f)
...
def build_greeting(mail_data, backend_file=BACKEND_DATA):
    ...
    try:
        ...
        try:
            ...
            for name in user_names:
                prefix = get_notes_prefix(name, BACKEND_DATA)
                if prefix:
            ...
            notes_emails = []
            for user in unique_users:
                email = get_notes_email(user, BACKEND_DATA)
                if email:
            ...
            # ✨ 生成問候語
            greeting = build_greeting(mail_data, BACKEND_DATA)
            logger.info(f"生成的問候語: {greeting}")
            ...
            send_mail(mail_data, mail_name, ccList, po_str, to_str, greeting, recipients_file=str(MAIL_RECIPIENTS_FILE))
...
# 路徑常數（MB_DIR / MB_CHANNELS_DIR / MB_VISITS_DIR / MB_LOG_FILE）已統一移至檔案開頭
...
def mb_channel_path(channel):
    return os.path.join(MB_CHANNELS_DIR, f'{current_site()}_{channel}.json')


def mb_visit_path(username):
    safe = str(username).replace('/', '_').replace('\\', '_')
    return os.path.join(MB_VISITS_DIR, f'{current_site()}_{safe}.json')
